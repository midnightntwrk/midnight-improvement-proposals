...
Launching Prime Stablecoin on Midnight requires that Compact smart contracts can (1) hash serialized `DepositIntent` messages using Ethereum-native Keccak-256 and (2) recover a signer address from an ECDSA signature over Secp256k1. These two primitives unlock the deposit intent lifecycle: serialization → hashing → signing → on-chain verification.

## Motivation

Prime Stablecoin's minting flow is initiated by a `DepositIntent` message that must be serialized off-chain, hashed, signed by an authorized party, and verified on-chain before minting proceeds. 

These Compact capabilities will allow for a `deposit intent`, as shown below to be processed by a Compact smart contract. As an example [this](https://gist.github.com/yvonnezhangc/205c2bf28ec7f80ee6dbc7a9001773d9) document details the steps partners must take to serialize, hash, sign, and validate a DepositIntent message that initiates the minting of stablecoin. 

## Specification

### Capability 1: Keccak-256 Hashing

Compact smart contracts must support hashing an Ethereum-native `DepositIntent` using Keccak-256.
