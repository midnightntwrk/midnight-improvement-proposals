...
(Per-node disk cost is discussed under `## Backwards Compatibility Assessment` "Archive-node disk growth"; retention is discussed under `## Performance Impact` "Mitigations and operator controls". Both rows are kept out of the table here to avoid duplication.)
...
One scope decision is recorded explicitly so a reader does not infer it from omission. The Compact ↔ Ledger event-schema sharing surface — what a Compact-side transcript verifier would need beyond byte-level transport — is cross-referenced from this MIP, not designed within it. The reasoning is in `## Event Schema and Compact Integration`: the schema-sharing protocol touches the Compact compiler, the ledger crate, and possibly the runtime ABI, which makes it a separable proposal whose scope would dilute this MIP's load-bearing per-block-event surface. The dependency is recorded in the frontmatter `Requires:` field and in the named section; the sibling MIP is the right venue for that design.

## Performance Impact

Depositing one runtime event per ledger event puts a small additional cost on the critical path of block authoring and block verification. This section quantifies that cost against the existing block-weight budget, identifies the regime in which it would become operationally significant, and names the mitigations and operator controls available. Numbers are envelopes drawn from existing sources; no fresh benchmarks are produced inside this MIP. Where a number is an estimate rather than a measured datapoint, it is marked `[estimate]`.

### Per-event weight envelope

`frame_system::deposit_event` performs a small, near-constant amount of work per call: an allocation, a SCALE-encode of the event payload, and a push onto the per-block `Events` vector that is later committed by the storage root. Substrate publishes a `frame_system::WeightInfo` benchmark for `deposit_event`-equivalent operations at the polkadot-sdk pin (`polkadot-stable2603`) the midnight-node depends on. The CPU cost is dominated by the SCALE-encode of the payload (linear in the encoded byte length) rather than by the storage write itself; the constant overhead per deposit is hundreds of nanoseconds CPU on a typical validator [estimate], on top of which the SCALE-encode runs at a few hundred MB/s on the same hardware [estimate]. The storage write is appended to a `StorageValue` that is reset at the next block, so it touches transient storage only — no trie-node creation cost beyond the per-block storage-root pass that already runs.

The wire shape this MIP commits to bounds the payload length deterministically. `LedgerEventSource` is `32 + 2 + 2 = 36` bytes; `content_tagged_bytes` is a length-prefixed `Vec<u8>` whose contents are bounded by the ledger crate's own serialiser plus the per-transaction byte limit (`transaction_byte_limit = 1 MiB`). At the typical event envelope from the source kb-research analysis (`~45 KiB of events per block` across all transactions) the SCALE-encode work is on the order of low microseconds [estimate] per block on the deposit critical path.

### Block-budget headroom analysis

The polkadot-sdk runtime exposes a `BlockWeights::max_block` budget that bounds the total weight of work an honest validator will accept on a single block. The midnight-node runtime configures this budget through the standard polkadot-sdk `BlockWeights` constants. Translating the per-event envelope above against that budget:

- **Typical envelope.** ~45 KiB of events per block; in the order of dozens to low hundreds of events per block; SCALE-encode and deposit cost on the order of low microseconds aggregate per block [estimate]. This corresponds to a small fraction of one per cent of `BlockWeights::max_block` consumed by event deposit on a typical block [estimate].
- **Heavy envelope.** 50–200 KiB of events per block, sustained; up to mid-thousands of events per block in a deploy-heavy or contract-log-heavy mix; SCALE-encode and deposit cost on the order of tens to low-hundreds of microseconds aggregate per block [estimate]. This corresponds to at most a few per cent of `BlockWeights::max_block` consumed by event deposit on a sustained-heavy block [estimate].

The acceptance criterion `M5 — Block-time impact bounded` in `## Path to Active` formalises the upper bounds against which the implementation PR is measured (≤ 5 % typical, ≤ 15 % deploy-heavy block-time increase). The envelope numbers here are consistent with M5's pre-implementation bound; the implementation PR is the authoritative source of measured numbers once its benchmark suite is integrated. No headroom percentage in this MIP body should be read as a measured datapoint; each is marked `[estimate]` and is intended to set expectations for the order of magnitude.

### Mitigations and operator controls

Three controls bound or absorb the event-deposit cost without further protocol change:

- **Per-event payload size bound.** The wire shape's `content_tagged_bytes` field is bounded by the ledger crate's own tagged-serialiser plus the per-transaction byte limit. A transaction that constructed a single oversized event would already be rejected as exceeding `transaction_byte_limit = 1 MiB`. No new bound is introduced; the existing resource-pricing model is the bound (see `## Security Considerations` "Archive growth as a resource-pricing concern").
- **Archive-node opt-in pruning.** Default-pruned nodes drop block-N events out of active state at the keep-window boundary; only archive operators carry the long-tail cost. The opt-in is the standard Substrate archive configuration; no new operator-facing flag is introduced (S1 in the source planning folder).
- **Fallback to mechanism (c) — pallet-controlled storage column.** If sustained operational experience under (a) shows the block-weight or archive-disk envelope is unsustainable, the wire shape is portable to a pallet-controlled storage column with bounded retention. The consumer-facing decoder (`LedgerEvent { source, content_tagged_bytes }`) is unchanged; the persistence layer moves. This is the documented escape hatch (see `## Rationale` mechanism (c)).

### What is and is not measured

The numbers above are envelopes, not measurements. Specifically:

- **From the source kb-research § 1 envelope analysis.** Per-block event-byte counts (`~45 KiB typical`, `50–200 KiB heavy`) and the resulting archive-disk growth (`~57 GiB/year typical`, `~263 GiB/year sustained heavy`) are envelope figures drawn from observed event-volume mixes; they are not fresh benchmarks.
- **From Substrate's published weights at `polkadot-stable2603`.** The constant overhead of `frame_system::deposit_event` and the SCALE-encode rate are taken from the polkadot-sdk benchmark suite documentation; the midnight-node runtime does not re-benchmark them. These figures apply on validator hardware comparable to a standard polkadot-stable2603 reference machine; deviations on different hardware shift the absolute numbers but not the relative envelope.
- **What is not measured inside this MIP.** No fresh `deposit_event` micro-benchmark on the midnight-node validator profile; no end-to-end TPS regression test against the implementation branch; no archive-disk-growth measurement against a sustained heavy workload. These are evidence that would be supplied by [`midnight-node` PR #1487](https://github.com/midnightntwrk/midnight-node/pull/1487)'s benchmark suite (criteria M5 and M8) at numbering time. Estimates marked `[estimate]` are placeholders for that evidence — editors and reviewers may require their replacement by measured numbers before this MIP advances beyond `Accepted`.
...
Measured performance evidence for the change — `deposit_event` micro-benchmarks, end-to-end block-time deltas, archive-disk-growth measurements — comes from [`midnight-node` PR #1487](https://github.com/midnightntwrk/midnight-node/pull/1487)'s benchmark suite (criteria M5 and M8) and is the canonical evidence behind the envelopes recorded in `## Performance Impact`.
