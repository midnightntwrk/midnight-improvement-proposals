...
Midnight's runtime exposes a `BlockWeights::max_block` budget (inherited from polkadot-sdk) bounding the total weight of work a validator will accept on a block. Translating the per-event envelope against that budget against the source kb-research § 1 event-volume figures:
