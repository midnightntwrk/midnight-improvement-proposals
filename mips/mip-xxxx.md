...
Transport is necessary but not sufficient for a separate class of consumer: a **Compact-side transcript verifier**. The Compact compiler / runtime, when verifying that a contract execution emitted the events its transcript declares, needs to evaluate emitted events against a schema both Compact and the ledger crate agree on. The current Compact ↔ Ledger surface is byte-level only: the ledger crate emits typed `EventDetails<D>` values, tagged-serialised at the boundary, and the Compact side does not today consume those types directly. There is no shared types crate, no published registry, and no runtime hook the Compact verifier can call to obtain an authoritative event-schema description. The byte-level boundary is what this MIP exposes; the schema-sharing surface a transcript verifier would compose against it is a distinct concern.

**Routing of the schema-sharing problem.** This MIP records the dependency and points at the work that resolves it; it does not design that work. A sibling MIP — referenced from this MIP's front matter as `Requires: MIP-???? — Compact ↔ Ledger event-schema sharing protocol [pre-numbering]` — is the right venue. That sibling MIP would specify the registry or shared-types crate holding the canonical schema, the mechanism by which Compact obtains it (compile-time vendoring, host-fn fetch, or in-chain storage), the versioning rules that govern Compact ↔ Ledger compatibility as the ledger's event taxonomy evolves, and the security implications of letting the Compact side reason about ledger-emitted event content beyond what the ledger's serialiser provides. These choices touch the Compact compiler, the ledger crate, and possibly the runtime ABI; folding them into the present MIP would dilute its load-bearing per-block event surface and obscure the scope of the schema-sharing work.

**What this MIP does and does not commit to.** The opaque tagged-byte payload is forward-compatible: new `EventDetails` variants or field additions appear as different bytes inside `content_tagged_bytes` and require no changes to the wrapper, the pallet event variant, or the runtime metadata (see `## Backwards Compatibility Assessment` Layer 2). That makes transport stable across ledger evolution. It does **not** by itself give Compact a mechanism to validate transcript-declared events without holding the same `midnight-ledger` version. Until the sibling MIP lands, consumers needing a Compact-side validation hook — rather than a transport-level decoder — are outside this MIP's scope. This MIP can transition to `Active` independent of the sibling MIP for the consumer set it commits to (Substrate-tooling consumers reading `frame_system::Events`); editors may gate `Active` on the sibling MIP at numbering time via the `Requires:` field if that is preferred.
...
The mechanism specified in `## Specification` is **(a)**. The decisive properties are light-client friendliness and tooling reach: `frame_system::Events` is part of the storage trie, so light clients verify any event through `state_getReadProof` without needing the full ledger state or trusting any specific indexer, and the same record decodes against the runtime's metadata in `polkadot.js`, `subxt`, and every Substrate-tooling integration teams have already built — the explicit UC5 promise in MPS-0007.

Mechanism **(b)** preserves the present-day arrangement: events remain derivable but only via replay. It demands the indexer be operationally available and version-pinned, and it leaves the UC1 / UC2 / UC3 / UC5 use cases unaddressed. It is the implicit baseline against which (a) is measured. Mechanism **(c)** adds a pallet-controlled retention policy — operators get a knob the pallet owns — at the cost of a new storage column, retention parameter, and custom subscription surface consumer libraries do not already understand. The wire shape this MIP commits to is portable to (c) without re-spec; if operator experience under (a) shows archive growth is unsustainable, a future MIP can switch persistence to (c) without changing the consumer-facing decoder. Mechanism **(d)** is a convenience wrapper, not an alternative: because `state_subscribeStorage([System.Events])` already provides real-time delivery decodable through any Substrate client, (d) is deferred and can ship in a follow-on without altering the wire shape.

Within mechanism (a), three sub-decisions warrant explicit rationale:

- **Opaque tagged bytes for `EventDetails<D>` rather than a SCALE-mirrored enum.** `EventDetails<D>` is `#[non_exhaustive]`; mirroring its variants in SCALE creates a maintenance ratchet on every ledger upgrade and risks silent drift. Carrying the ledger's own `Tagged` byte form makes the wrapper forward-compatible — consumers decode against the ledger version they ship with, and the runtime ABI does not move when the event taxonomy does.
- **One pallet-event variant per ledger event rather than one variant per block carrying a `Vec`.** `subxt` and equivalent clients filter events by `(pallet_index, variant_index)` *before* SCALE-decoding the payload. A block-level `Vec<LedgerEvent>` variant forces clients to decode the whole vec before filtering, defeating the typed event channel's filtering granularity. One variant per event aligns with the existing pallet variants (`ContractCall`, `ContractDeploy`, `ContractMaintain`, …) which each fire once per occurrence.
- **A new `TransactionAppliedStateRootV2` sibling struct rather than extending the existing struct in place.** SCALE does not silently ignore trailing bytes on decode; extending the existing struct would break replay against WASM runtimes compiled against the old encoding. A sibling struct returned by `#[version(3)]` leaves historical replay paths byte-stable, following the precedent already in tree (`ledger/src/host_api/ledger_7.rs::apply_transaction` v1 vs v2).

One scope decision is recorded explicitly. The Compact ↔ Ledger event-schema sharing surface — what a Compact-side transcript verifier would need beyond byte-level transport — is cross-referenced from this MIP rather than designed within it (see `## Event Schema and Compact Integration`). That separation keeps the present MIP's per-block event surface load-bearing and routes the schema-sharing work to the sibling MIP named in the frontmatter `Requires:` field.

## Performance Impact

Depositing one runtime event per ledger event puts a small additional cost on the critical path of block authoring and verification. This section quantifies that cost against the existing block-weight budget, names the mitigations available, and is explicit about what is estimate rather than measurement. Numbers are envelopes drawn from existing sources; no fresh benchmarks are produced inside this MIP, and every quantitative claim below is marked `[estimate]`.
...
Three controls bound or absorb the event-deposit cost without further protocol change. (i) **Per-event payload size bound:** the existing per-transaction byte limit already caps the bytes any transaction can produce, so no new bound is introduced. (ii) **Archive opt-in pruning:** default-pruned nodes drop block-N events at the keep-window boundary; only archive operators carry the long-tail cost, via the standard Substrate archive configuration. (iii) **Fallback to mechanism (c):** if sustained operational experience shows the envelope is unsustainable, the wire shape is portable to a pallet-controlled storage column with bounded retention — the consumer-facing decoder is unchanged, only the persistence layer moves.
