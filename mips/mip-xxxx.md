...
This MIP proposes a candidate solution to [MPS-0007: Node-Side Visibility of Ledger Events](../mps/mps-0007-node-event-visibility.md). The Midnight node receives a stream of per-transaction events from the ledger crate during transaction application but discards them at the ledger-bridge boundary before they reach any observable surface. This proposal specifies their publication on the `frame_system::Events` channel Midnight inherits from Substrate, wrapped in a new `pallet_midnight::Event::LedgerEvent` variant whose outer shape is stable across ledger-crate upgrades. A consumer using Substrate-compatible tooling — `polkadot.js`, `subxt`, or the `state_subscribeStorage` RPC — receives the events in the order they were produced, decoded against the pallet's metadata, with the per-event content carried as the ledger crate's own tagged-byte serialisation so that downstream decoders evolve with the ledger crate rather than with the runtime ABI. The change is non-protocol — it adds no new gas costs, no new consensus rules, and no new block-validity constraints — and ships in a normal runtime upgrade alongside a versioned host-fn (`apply_transaction` `#[version(3)]`) that preserves replay against historical WASM runtimes.

## Motivation

[MPS-0007: Node-Side Visibility of Ledger Events](../mps/mps-0007-node-event-visibility.md) is the upstream Midnight Problem Statement that this MIP addresses. MPS-0007 frames the gap in detail: the ledger crate's `state.apply(verified_tx, ctx)` returns a `TransactionResult` whose `Success` and `PartialSuccess` variants carry the typed event records produced during transaction execution. Those event vectors are consumed internally by the node for dust-state accounting — `update_dust_from_events` at `ledger/helpers/src/versions/common/context.rs:226` walks them to maintain the per-account dust ledger — but they are not exposed externally: no pallet event variant, no RPC method, and no storage surface carries them past the ledger-bridge boundary. The pallet therefore deposits FRAME events derived from the bridge's settled fields (`state_root`, `tx_hash`, call/deploy/maintain addresses, claim rewards, unshielded UTXO deltas) but emits nothing reflecting the contract-internal events the network actually computed, so consumers that need to know what happened in each block have no path to ask the node.

Midnight is a Cardano partner chain. Its runtime composes AURA block authorship, GRANDPA finality, and a BEEFY gadget (BEEFY is present specifically to back the Cardano-bridge security surface), and the runtime hosts `pallet_cnight_observation` for the cNIGHT → DUST Cardano-observation inherent and `pallet_c2m_bridge` for Cardano-to-Midnight transfers, alongside the partner-chains framework vendored into midnight-node (its session and committee-selection pallets). `pallet_midnight` is one tenant of that runtime; the changes proposed here touch only the ledger-bridge surface and the pallet's own event enum, leaving the other tenants unaffected.

The consequence for downstream consumers is the one MPS-0007 enumerates as use cases UC1 through UC5: contract authors cannot see the events their own contracts emit through `polkadot.js`, block explorers cannot display per-event payloads without standing up a separate indexer, light-client DApps have no path to subscribe to events without trusting a hosted service, operators have no lightweight visibility for incident response, and cross-tooling integrations break at the Midnight boundary because the Substrate-compatible event pipeline Midnight's runtime exposes is empty. Today the only consumer that recovers these events is the `midnight-indexer`, which does so by re-running each transaction against its own copy of the ledger crate.
...
Ledger-emitted events are surfaced through the `frame_system::Events` storage value Midnight's runtime inherits from Substrate. `pallet_midnight::Event` gains one new variant, appended to the existing enum:
...
`LedgerEventSource` mirrors the ledger crate's `EventSource` triple verbatim: the 32-byte transaction hash plus two `u16` segment indices that identify the call slot within the transaction (logical segment for the call boundary, physical segment for the dispatch slot). `content_tagged_bytes` carries the ledger crate's own `Tagged` byte serialisation of the typed `EventDetails<D>` variant — `ZswapInput`, `ZswapOutput`, `ContractDeploy`, `ContractLog`, `ParamChange`, `DustInitialUtxo`, `DustGenerationDtimeUpdate`, or `DustSpendProcessed` at the time of writing; the enum is `#[non_exhaustive]` so future variants are admitted without ABI change — opaque to SCALE and decoded by the matching `midnight-ledger` version on the consumer side. The canonical variant list is `midnight-ledger/ledger/src/events.rs`.
...
`frame_system::Events` is reset at every block's `initialize_block_impl`. On a default-pruned node, block-N events leave active state when block N falls out of the keep-window. Archive operators retain them via the same archive configuration (Substrate-inherited) that retains every other event in Midnight's runtime; this MIP introduces no new operator-facing flag. The wire shape is portable to a future pallet-controlled storage column with bounded retention should operational experience show archive-disk growth is unsustainable.

### Pallet ABI version transition

The behaviour ships behind a new `#[version(3)]` of `apply_transaction` and a `#[version(2)]` of `apply_system_transaction` on both `host_api/ledger_7.rs` and `host_api/ledger_8.rs`. The pre-existing `#[version(1)]` and `#[version(2)]` of `apply_transaction` stay in tree to support historical replay against WASM runtimes compiled before this MIP's activation. The runtime upgrade that introduces the new variant increments the runtime's `spec_version`; the Substrate-inherited runtime-upgrade machinery routes each WASM blob to the host-fn version it was compiled against. The pallet's `Event` enum gains one variant at the tail of the enum; the existing variants retain their declared positions and `#[codec(index)]` values.
...
Midnight's downstream consumer model operates on **finalised** data. The present-day consumer of ledger event content is the indexer, which serves wallets via its API (`midnight-indexer/README.md`); light-client DApps and cross-chain bridges are anticipated consumers whose architectures this MIP's design supports through the storage-root-proof model. The reconciliation between per-block event publication and the finalised-only consumer model is a consumer-side composition of two primitives Midnight's runtime inherits from Substrate. The canonical flow is:
...
This is the same shape Midnight's existing indexer uses for finalised-block event subscription, now applied to the new `LedgerEvent` variant. Consumers that already read pallet events on finalised heads — the existing indexer is one such consumer — keep their architecture; the change is that `pallet_midnight::Event::LedgerEvent` now carries content the indexer previously had to recover by re-running `state.apply` against its own copy of the ledger crate. The flow above is the minimum-cost path this MIP enables; existing consumers — the indexer included — MAY continue re-execution if they want richer post-execution context, since the indexer's finalised-blocks subscription (`midnight-indexer/chain-indexer/src/infra/subxt_node.rs:118`, subxt `stream_blocks()` is finalised-only) and per-transaction `ledger_state.apply()` loop (`midnight-indexer/chain-indexer/src/domain/ledger_state.rs:119-144`) remain valid against the new pallet metadata.

Consumers that want to observe events on the best chain rather than the finalised chain can subscribe to `state_subscribeStorage([System.Events])` directly and accept the reorg risk that comes with reading pre-finality state. That is the trade-off Midnight already inherits from Substrate for any state read on non-finalised blocks; the MIP does not invent a new path and does not change it. High-value consumers that require both finality and a streaming surface compose `chain_subscribeFinalizedHeads` with per-block `state_getStorage` reads, as described above.
...
- **(a) `frame_system::Events` via a new pallet event variant.** The mechanism specified above. Events ride the FRAME event channel Midnight's runtime already exposes, decoded by any consumer that already reads pallet events through Substrate-compatible tooling.
...
The mechanism specified is **(a)**. The decisive properties are light-client friendliness and tooling reach: Midnight's `frame_system::Events` is storage-trie-committed, so light clients verify any event via `state_getReadProof` without full ledger state or a trusted indexer, and the same record decodes against runtime metadata in `polkadot.js`, `subxt`, and every Substrate-compatible tooling integration — the explicit UC5 promise in MPS-0007.

Mechanism **(b)** preserves the status quo and leaves UC1 / UC2 / UC3 / UC5 unaddressed. Mechanism **(c)** adds a pallet-owned retention knob at the cost of a new storage column and a custom subscription surface consumer libraries do not already understand; the wire shape this MIP commits to is portable to (c) without re-spec if archive growth under (a) proves unsustainable. Mechanism **(d)** is a convenience wrapper, not an alternative: `state_subscribeStorage([System.Events])` already provides real-time delivery, so (d) is deferred to a follow-on without altering the wire shape.
...
Midnight's runtime uses the polkadot-sdk default block weights without custom tuning: `midnight-node/runtime/src/lib.rs:312-316` constructs `BlockWeights::with_sensible_defaults(2 * WEIGHT_REF_TIME_PER_SECOND, NORMAL_DISPATCH_RATIO)` with `NORMAL_DISPATCH_RATIO = 75%`. The resulting `BlockWeights::max_block` bounds the total weight of work a validator will accept on a block. Translating the per-event envelope against that budget against the source kb-research § 1 event-volume figures:
...
The pre-existing pallet event deposits at `pallets/midnight/src/lib.rs:391-425` (`ContractCall`, `ContractDeploy`, `ContractMaintain`, claim rewards, unshielded UTXO deltas) are not included in `get_tx_weight()` — Midnight's established pattern is that event emission sits outside the extrinsic weight budget, with deposit cost absorbed against the block-budget headroom analysed above. The new `LedgerEvent` deposits follow this pattern.

### Mitigations and operator controls

Three controls absorb the event-deposit cost without further protocol change. (i) The existing per-transaction byte limit already caps producible bytes; no new bound. (ii) Default-pruned nodes drop block-N events at the keep-window boundary, leaving only archive operators with the long-tail cost. (iii) The wire shape is portable to mechanism (c) — a pallet-controlled storage column with bounded retention — if the envelope proves unsustainable; only the persistence layer moves.

### What is and is not measured

Numbers above are envelopes, not measurements. Event-byte counts and archive-disk-growth figures (`~57 GiB/year typical`, `~263 GiB/year sustained heavy`) come from the source kb-research § 1 envelope analysis; the `deposit_event` constant overhead and SCALE-encode rate come from the polkadot-sdk benchmark documentation at `polkadot-stable2603`. The fresh `deposit_event` micro-benchmark on the midnight-node validator profile, end-to-end TPS regression, and sustained-heavy archive-disk-growth measurement are supplied by [`midnight-node` PR #1487](https://github.com/midnightntwrk/midnight-node/pull/1487)'s benchmark suite (criteria M5 and M8) at numbering time; `[estimate]` markers may be replaced by measured numbers before this MIP advances beyond `Accepted`.
...
**Scope relative to other pallet-event channels.** Other tenants of Midnight's runtime emit pallet events independent of `pallet_midnight::Event::LedgerEvent` — `pallet_cnight_observation` and `pallet_c2m_bridge` for the Cardano bridge, `pallet_partner_chains_session` for partner-chains session rotation, and the governance pallets — and continue to do so unchanged. This MIP scopes to ledger events from `state.apply`; those other channels are unaffected, their consumers are unaffected, and the new variant adds to rather than displaces any of them on `frame_system::Events`.

**Archive-node disk growth.** The one *operational* compatibility concern is archive-node disk consumption. Six-second blocks at the typical event envelope (~45 KiB per block) produce roughly 6.5 MiB per hour, and at the sustained-heavy envelope (50–200 KiB per block) roughly 30 MiB per hour — on the order of 57 GiB per year typical and 263 GiB per year sustained heavy. The figures are modest (a one-terabyte archive volume absorbs a decade of typical load) but archive operators should account for them when sizing storage. Default-pruned nodes are unaffected: `frame_system::Events` is reset at every block. Archive operators opt into the cost via the same archive configuration (Substrate-inherited) that retains every other event in Midnight's runtime; this MIP introduces no new flag. If sustained operational experience indicates the envelope is unsustainable, mechanism (c) from the Rationale is the documented fallback, and the wire shape this MIP commits to is portable to it without consumer-side churn.
...
**Archive growth as a resource-pricing concern.** A pathological transaction — a large `ContractDeploy` near the per-transaction byte ceiling, or a contract emitting high-volume `ContractLog` events — inflates archive-node disk usage at the heavy end of the envelope documented in Backwards Compatibility. The mitigation is the existing resource-pricing model: the per-transaction byte limit (`transaction_byte_limit = 1 MiB`) and the synthetic block-usage budget (`block_usage = 200_000`) already bound per-block event volume. This MIP does not change those bounds and does not let an adversary bypass any existing limit; the cost paid at `state.apply` time is unchanged, only the post-bridge fate of the produced events differs. Default-pruned nodes have no new exposure. Archive operators inherit the cost via the same retention configuration they already use for every other pallet event; the option-(c) escape hatch is documented as the remediation path if sustained operational experience surfaces an unmanageable envelope.

**Light-client trust assumptions.** A light client that subscribes to `state_subscribeStorage([System.Events])` over a single full-node connection trusts that node not to omit or fabricate event records. The storage-root trust model Midnight inherits from Substrate addresses this: `frame_system::Events` is part of the storage trie, and the trie's root is committed in the block header. Any subscriber that requires more than the connecting node's word can request a Merkle proof via `state_getReadProof` for the `System.Events` key against a specific block hash, and verify the events against the header-committed storage root. This is the same trust model already in place for every other event in Midnight's runtime; the MIP introduces no new trust assumption. A cross-chain bridge or any high-value consumer should consume events through storage proofs rather than bare subscription — the same guidance that applies to every other event channel in Midnight's runtime.
...
No new external dependencies: the change reuses the FRAME event machinery Midnight's runtime inherits from Substrate, the established host-fn versioning pattern (v1 → v2 of `apply_transaction`), and the existing pallet event deposit path. Activation requires only a standard runtime upgrade incrementing `spec_version`.
