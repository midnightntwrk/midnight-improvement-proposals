...
Not part of this MPS: Phase 2 (future work) extends the model with **private events** — `proven` blocks for commitment binding and `encrypt_for` for encrypted delivery — enabling privacy-preserving event patterns. Phase 2 is scoped in Part 2 of this document.
...
- **Not Part of this MPS: Phase 2:** `@private` field annotation, `@topic` annotation for filtering, client SDK encryption/decryption, `@zkp` ZK circuit integration.
...
- **Not part of this MPS: Phase 2:** Topics column for private event filtering. Wallet-specific decryption with session isolation. `RelevantContractEvent` table for per-wallet relevance.
